...
public class RequestAnalyzer {
	...
	public void handleRequest(Request r) throws IOException {
		...
		if (type != null) {
			...
			if (rq.charAt(firstSpace + 1) != '/') {
				throw new CanNotCompleteTheRequestException(
						"Need / to begin request after " + type.name() + " : " + rq);
			}
			int secondSpace = rq.indexOf(' ', firstSpace + 1);
			if (secondSpace < 0) {
				throw new CanNotCompleteTheRequestException(
						"Need \" HTTP/1.0\" or \" HTTP/1.1\" at the end of the request : " + rq);
			}
	...
	public ServletFileI getRs() {
		return rs;
	}

	public void setRs(ServletFileI rs) {
		this.rs = rs;
	}
